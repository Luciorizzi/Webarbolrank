  ...
  return (
    <article className={`group relative grid min-w-0 gap-5 overflow-hidden rounded-3xl border p-5 transition sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:p-6 ${participant.position === 1 ? "border-amber-400/35 bg-amber-400/[0.07]" : "border-white/9 bg-white/[0.035] hover:border-white/16"}`}>
      ...
      <div className="min-w-0">
        ...
        <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-zinc-500"><span>{numberFormatter.format(participant.contributors)} personas contribuyeron</span><span className="text-amber-400">+{formatImpactKg(participant.recentImpactUnits)} esta semana</span>{milestone && <span className="rounded-full border border-amber-400/20 px-2 py-0.5 text-amber-200">{milestone.name}</span>}</div>
        ...
        <ImpactProgress impactUnits={participant.impactUnits} compact />
      </div>
