...
export function ImpactUnitCounter({ value, onChange }: ImpactUnitCounterProps) {
  return (
    <div aria-label="Kilogramos de alimento a aportar">
      ...
      <button type="button" onClick={() => onChange(Math.max(KG_PER_DONATION_UNIT, value - KG_PER_DONATION_UNIT))} disabled={value === KG_PER_DONATION_UNIT} aria-label="Quitar una unidad de aporte" className="grid size-12 place-items-center rounded-full border border-white/12 text-xl text-white transition hover:border-amber-400/50 disabled:cursor-not-allowed disabled:opacity-30">−</button>
      ...
      <button type="button" onClick={() => onChange(Math.min(MAX_IMPACT_UNITS, value + KG_PER_DONATION_UNIT))} disabled={value === MAX_IMPACT_UNITS} aria-label="Agregar una unidad de aporte" className="grid size-12 place-items-center rounded-full border border-white/12 text-xl text-white transition hover:border-amber-400/50 disabled:opacity-30">+</button>
      </div>
